...
def debug():
    """Debug com visualização da imagem de busca"""
    try:
        # Verificar se o arquivo existe
        image_path = 'sapp3.png'
        if not os.path.exists(image_path):
            print(f"❌ Arquivo '{image_path}' não encontrado!")
            print(f"   Pasta atual: {os.getcwd()}")
            return
        
        # Abrir e mostrar a imagem que está sendo usada para busca
        print(f"📸 Abrindo imagem de referência: {image_path}")
        img = Image.open(image_path)
        img.show()  # Abre a imagem no visualizador padrão do Windows
        
        print(f"   Tamanho da imagem: {img.size[0]}x{img.size[1]} pixels")
        print(f"   Modo de cor: {img.mode}")
        
        # Agora tentar localizar
        print("\n🔍 Procurando na tela...")
        posnpc = pyautogui.locateCenterOnScreen(image_path, confidence=0.7)
        
        if posnpc:
            print(f"✅ Imagem encontrada em: {posnpc}")
        else:
...
#     posnpc = pyautogui.locateCenterOnScreen('sapp3.png')
#     print("Pos npc: ", posnpc)
